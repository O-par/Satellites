...
void dpper(double e3, double ee2, double peo, double pgho, double pho,
  ...
  /* --------------------- local variables ------------------------ */
  double alfdp, betdp, cosip, cosop, dalf, dbet, dls, f2, f3, pe, pgh, ph, pinc,
      pl, sel, ses, sghl, sghs, shll, shs, sil, sinip, sinop, sinzf, sis, sll,
      sls, xls, xnoh, zf, zm, zel, zes, znl, zns;
...
void dscom(double epoch, double ep, double argpp, double tc, double inclp,
  ...
  /* --------------------- local variables ------------------------ */
  int lsflg;
  double a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, betasq, cc, ctem, stem, x1,
      x2, x3, x4, x5, x6, x7, x8, xnodce, xnoi, zcosg, zcosgl, zcosh, zcoshl,
      zcosi, zcosil, zsing, zsingl, zsinh, zsinhl, zsini, zsinil, zx, zy;
...
void dsinit(double tc, double xpidot, ElsetRec *rec) {
  ...
  double ainv2, aonv = 0.0, cosisq, eoc, f220, f221, f311, f321, f322, f330,
                f441, f442, f522, f523, f542, f543, g200, g201, g211, g300,
                g310, g322, g410, g422, g520, g521, g532, g533, ses, sgs, sghl,
                sghs, shs, shll, sis, sini2, sls, temp, temp1, theta, xno2, q22,
                q31, q33, root22, root44, root54, rptim, root32, root52, x2o3,
                znl, emo, zns, emsqo;
  ...
  rec->dmdt =
      sls - znl * rec->s3 * (rec->z1 + rec->z3 - 14.0 - 6.0 * rec->emsq);
  ...
  if (rec->irez != 0) {
    ...
    if (rec->irez == 2) {
      ...
      if (rec->em <= 0.65) {
        ...
        g310 = -19.302 + 117.3900 * rec->em - 228.4190 * rec->emsq +
               156.5910 * eoc;
        g322 = -18.9068 + 109.7927 * rec->em - 214.6334 * rec->emsq +
               146.5816 * eoc;
        g410 = -41.122 + 242.6940 * rec->em - 471.0940 * rec->emsq +
               313.9530 * eoc;
        g422 = -146.407 + 841.8800 * rec->em - 1629.014 * rec->emsq +
               1083.4350 * eoc;
        g520 = -532.114 + 3017.977 * rec->em - 5740.032 * rec->emsq +
               3708.2760 * eoc;
      } else {
        g211 =
            -72.099 + 331.819 * rec->em - 508.738 * rec->emsq + 266.724 * eoc;
        g310 = -346.844 + 1582.851 * rec->em - 2415.925 * rec->emsq +
               1246.113 * eoc;
        g322 = -342.585 + 1554.908 * rec->em - 2366.899 * rec->emsq +
               1215.972 * eoc;
        g410 = -1052.797 + 4758.686 * rec->em - 7193.992 * rec->emsq +
               3651.957 * eoc;
        g422 = -3581.690 + 16178.110 * rec->em - 24462.770 * rec->emsq +
               12422.520 * eoc;
        if (rec->em > 0.715)
          g520 = -5149.66 + 29936.92 * rec->em - 54087.36 * rec->emsq +
                 31324.56 * eoc;
        else
          g520 = 1464.74 - 4664.75 * rec->em + 3763.64 * rec->emsq;
      }
      if (rec->em < 0.7) {
        g533 = -919.22770 + 4988.6100 * rec->em - 9064.7700 * rec->emsq +
               5542.21 * eoc;
        g521 = -822.71072 + 4568.6173 * rec->em - 8491.4146 * rec->emsq +
               5337.524 * eoc;
        g532 = -853.66600 + 4690.2500 * rec->em - 8624.7700 * rec->emsq +
               5341.4 * eoc;
      } else {
        g533 = -37995.780 + 161616.52 * rec->em - 229838.20 * rec->emsq +
               109377.94 * eoc;
        g521 = -51752.104 + 218913.95 * rec->em - 309468.16 * rec->emsq +
               146349.42 * eoc;
        g532 = -40023.880 + 170470.89 * rec->em - 242699.48 * rec->emsq +
               115605.82 * eoc;
      }
      ...
      f542 = 29.53125 * rec->sinim *
             (2.0 - 8.0 * rec->cosim +
              cosisq * (-12.0 + 8.0 * rec->cosim + 10.0 * cosisq));
      f543 = 29.53125 * rec->sinim *
             (-2.0 - 8.0 * rec->cosim +
              cosisq * (12.0 + 8.0 * rec->cosim - 10.0 * cosisq));
      xno2 = rec->nm * rec->nm;
      ainv2 = aonv * aonv;
      ...
      rec->xlamo =
          fmod(rec->mo + rec->nodeo + rec->nodeo - theta - theta, twopi);
      rec->xfact = rec->mdot + rec->dmdt +
                   2.0 * (rec->nodedot + rec->dnodt - rptim) - rec->no_unkozai;
      rec->em = emo;
      rec->emsq = emsqo;
    }
    ...
    if (rec->irez == 1) {
      ...
      f311 = 0.9375 * rec->sinim * rec->sinim * (1.0 + 3.0 * rec->cosim) -
             0.75 * (1.0 + rec->cosim);
      f330 = 1.0 + rec->cosim;
      ...
      rec->xfact = rec->mdot + xpidot - rptim + rec->dmdt + rec->domdt +
                   rec->dnodt - rec->no_unkozai;
    }
...
bool sgp4init(char opsmode, ElsetRec *satrec) {
  ...
  double cc1sq, cc2, cc3, coef, coef1, cosio4, eeta, etasq, perige, pinvsq,
      psisq, qzms24, sfour, tc, temp, temp1, temp2, temp3, tsi, xpidot, xhdot1,
      qzms2t, ss, x2o3, r[3], v[3], delmotemp, qzms2ttemp, qzms24temp;
  ...
  satrec->am = satrec->em = satrec->im = satrec->Om = satrec->mm = satrec->nm =
      0.0;
  ...
  if ((satrec->omeosq >= 0.0) || (satrec->no_unkozai >= 0.0)) {
    ...
    if (satrec->ecco > 1.0e-4)
      cc3 = -2.0 * coef * tsi * satrec->j3oj2 * satrec->no_unkozai *
            satrec->sinio / satrec->ecco;
    satrec->x1mth2 = 1.0 - satrec->cosio2;
    satrec->cc4 =
        2.0 * satrec->no_unkozai * coef1 * satrec->ao * satrec->omeosq *
    ...
    satrec->cc5 = 2.0 * coef1 * satrec->ao * satrec->omeosq *
    ...
    satrec->mdot = satrec->no_unkozai +
                   0.5 * temp1 * satrec->rteosq * satrec->con41 +
                   0.0625 * temp2 * satrec->rteosq *
                       (13.0 - 78.0 * satrec->cosio2 + 137.0 * cosio4);
    satrec->argpdot =
        -0.5 * temp1 * satrec->con42 +
    ...
    if (fabs(satrec->cosio + 1.0) > 1.5e-12)
      satrec->xlcof = -0.25 * satrec->j3oj2 * satrec->sinio *
                      (3.0 + 5.0 * satrec->cosio) / (1.0 + satrec->cosio);
    else
      satrec->xlcof = -0.25 * satrec->j3oj2 * satrec->sinio *
                      (3.0 + 5.0 * satrec->cosio) / temp4;
    ...
    if ((2 * pi / satrec->no_unkozai) >= 225.0) {
      ...
      dscom(epoch, satrec->ecco, satrec->argpo, tc, satrec->inclo,
            satrec->nodeo, satrec->no_unkozai, satrec);
      ...
      dpper(satrec->e3, satrec->ee2, satrec->peo, satrec->pgho, satrec->pho,
            ...
            satrec->zmol, satrec->zmos, satrec->init, satrec,
    ...
    if (satrec->isimp != 1) {
      ...
      satrec->d4 = 0.5 * temp * satrec->ao * tsi *
                   (221.0 * satrec->ao + 31.0 * sfour) * satrec->cc1;
...
bool sgp4(ElsetRec *satrec, double tsince, double *r, double *v) {
  ...
  double axnl, aynl, betal, cnod, cos2u, coseo1, cosi, cosip, cosisq, cossu,
      cosu, delm, delomg, ecose, el2, eo1, esine, argpdf, pl,
      mrt = 0.0, mvt, rdotl, rl, rvdot, rvdotl, sin2u, sineo1, sini, sinip,
      sinsu, sinu, snod, su, t2, t3, t4, tem5, temp, temp1, temp2, tempa, tempe,
      templ, u, ux, uy, uz, vx, vy, vz, xinc, xincp, xl, xlm, xmdf, xmx, xmy,
      nodedf, xnode, tc, x2o3, vkmpersec, delmtemp;
  ...
  if (satrec->isimp != 1) {
    ...
    tempe = tempe +
            satrec->bstar * satrec->cc5 * (sin(satrec->mm) - satrec->sinmao);
  ...
  if (satrec->method == 'd') {
    dpper(satrec->e3, satrec->ee2, satrec->peo, satrec->pgho, satrec->pho,
          ...
          satrec->xgh2, satrec->xgh3, satrec->xgh4, satrec->xh2, satrec->xh3,
          satrec->xi2, satrec->xi3, satrec->xl2, satrec->xl3, satrec->xl4,
          satrec->zmol, satrec->zmos, 'n', satrec, satrec->operationmode);
...
void getgravconst(int whichconst, ElsetRec *rec) {
  ...
  case wgs72:
    ...
    rec->radiusearthkm = 6378.135; // km
    rec->xke = 60.0 / sqrt(rec->radiusearthkm * rec->radiusearthkm *
                           rec->radiusearthkm / rec->mu);
    rec->tumin = 1.0 / rec->xke;
    rec->j2 = 0.001082616;
    rec->j3 = -0.00000253881;
    rec->j4 = -0.00000165597;
...
double gstime(double jdut1) {
  ...
  temp =
      fmod(temp * deg2rad / 240.0, twopi); // 360/86400 = 1/240, to deg, to rad
